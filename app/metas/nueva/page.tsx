    ...
    <main className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-lg mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6 space-y-6">
          ...
          <div>
            <label className="block text-sm font-bold text-gray-700 mb-2">¿Por qué quieres lograrlo? 💭</label>
            <textarea
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-3 mb-4">
              {[
                { id: 'daily', emoji: '📆', label: 'Todos los días' },
                { id: 'weekly', emoji: '📅', label: 'Una vez por semana' },
                { id: 'custom', emoji: '✏️', label: 'Días específicos' },
              ].map(f => (
                <button
                  key={f.id}
                  onClick={() => setFrequency(f.id)}
                  className={`p-3 rounded-xl border-2 text-center transition-all ${frequency === f.id ? 'border-orange-500 bg-orange-50' : 'border-gray-100 hover:border-gray-200'}`}
                >
              ...
              <div className="flex gap-2 justify-center mb-4">
                {diasSemana.map(dia => (
                  <button
                    ...
                    className={`w-10 h-10 rounded-full font-bold text-sm transition-all ${frequencyDays.includes(dia.id) ? 'bg-orange-500 text-white' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}
                  >
                    {dia.label}
                  </button>
                ))}
          ...
          <div>
            <label className="block text-sm font-bold text-gray-700 mb-2">⏰ Hora de recordatorio (opcional)</label>
            <input
              type="time"
              className="w-full border rounded-xl px-4 py-3 text-gray-800 focus:outline-none focus:ring-2 focus:ring-orange-300"
              value={reminderTime}
